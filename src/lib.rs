...
impl fmt::Display for Sudoku {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        for collum in 0..9 {
            for row in 0..9 {
                write!(f, "{} ", self.get_number(row, collum)).expect("Internal Formatting Error");
                if row % 3 == 2 {
                    write!(f, " ").expect("Internal Formatting Error");
                }
            }
            write!(f, "\n").expect("Internal Formatting Error");
            if collum % 3 == 2 {
                write!(f, "\n").expect("Internal Formatting Error");
            }
        }

        write!(f, "Moves: {:#?} ", self.moves).expect("Internal Formatting Error");
        Ok(())
    }
}
...
impl Sudoku {
    ...
    pub const UNSOLVED_SUDOKU_FILES: [&str; 8] = [
